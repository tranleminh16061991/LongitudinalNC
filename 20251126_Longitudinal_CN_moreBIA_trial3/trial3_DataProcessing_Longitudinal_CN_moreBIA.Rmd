...
df_pre_imputed<- df_pre_imputed%>%
  mutate(
    ...
    R5_upper   = (ra_r5   + la_r5)   / 2,
    R5_lower   = (rl_r5   + ll_r5)   / 2,
    R250_upper = (ra_r250 + la_r250) / 2,
    R250_lower = (rl_r250 + ll_r250) / 2,
    ...
    # --- ĐOẠN MỚI: biến toàn thân theo bài Frontiers ---
    #BMR_m = bmr * (ffm^(-0.25)),
    #BSA   = (1/6) * (weight * (height/100))^0.5,
    #Vol_BSA = weight / BSA,
    #Ri_Re_upper = (R_upper_250 - R_upper_5) / R_upper_5,
    #Ri_Re_lower = (R_lower_250 - R_lower_5) / R_lower_5,
...
```{r}
# ----------- (e) Flag outlier và tạo dataset full/clean trên dữ liệu đã impute -----------

# Các biến dùng để tính z-score và flag outlier
...
# Làm việc trên dữ liệu đã impute
df2 <- df_imputed

# Chỉ giữ những biến outlier thực sự tồn tại trong df2
vars_exist <- vars_check_outlier[vars_check_outlier %in% names(df2)]

# Tính z-score và flag outlier (|z| > 3 trên bất kỳ biến nào trong vars_exist)
df2 <- df2 %>%
  dplyr::mutate(
    dplyr::across(all_of(vars_exist), zscore_func, .names = "{.col}_z")
  ) %>%
  dplyr::mutate(
    outlier_extreme = dplyr::if_else(
      rowSums(dplyr::across(dplyr::ends_with("_z"), ~ abs(.) > 3), na.rm = TRUE) > 0,
      1L, 0L
    )
  )

# Dataframe đầy đủ (có flag) và dataframe đã loại outlier
df_full  <- df2
df_clean <- df2 %>% dplyr::filter(outlier_extreme == 0)
...
cat("Số đo trung bình mỗi subject (full):",
    round(nrow(df_full)  / length(unique(df_full$USUBJID)),  2), "\n")
cat("Số đo trung bình mỗi subject (clean):",
    round(nrow(df_clean) / length(unique(df_clean$USUBJID)), 2), "\n")

# Biến chính để báo cáo missing: 6 nhận thức + toàn bộ vars_check_outlier
vars_main <- c(
  "MMSE","SNSB_attention","SNSB_language",
  "SNSB_visuospatial","SNSB_memory","SNSB_frontal",
  vars_check_outlier
)

# Chỉ giữ các biến thực sự tồn tại
vars_main <- unique(vars_main[vars_main %in% names(df_full)])
...
# ---------- Xuất danh sách outlier detail ----------

outlier_rows <- df_full %>% dplyr::filter(outlier_extreme == 1)
  ...
  outlier_info <- outlier_rows %>%
    ...
    dplyr::mutate(
      outlier_vars = paste(
        vars_exist[
          unlist(dplyr::across(
            dplyr::all_of(paste0(vars_exist, "_z")),
            ~ abs(.) > 3
          ))
        ],
        collapse = ", "
      ),
      max_zscore = max(unlist(dplyr::across(dplyr::ends_with("_z"))), na.rm = TRUE)
    ) %>%
    dplyr::ungroup()
}
write.csv(outlier_info, "outlier_cases_detail_LOG.csv", row.names = FALSE)

# ---------- Báo cáo biến còn missing (toàn bộ biến) ----------

missing_report_nonzero <- sapply(df_full, function(x) sum(is.na(x)))
missing_report_nonzero <- data.frame(
  Variable      = names(missing_report_nonzero),
  Missing_Count = as.integer(missing_report_nonzero)
)
missing_report_nonzero <- missing_report_nonzero %>%
  dplyr::filter(Missing_Count > 0)
# write.csv(missing_report_nonzero, "missing_cases_report_LOG.csv", row.names = FALSE)

# ---------- Tần suất mỗi biến bị flag outlier ----------
...
if (nrow(outlier_info) == 0) {
  ...
  outlier_var_vec <- outlier_var_vec[outlier_var_vec != ""]
